...
public class CacheClearActivity extends Activity {
            ...
            switch (msg.what) {
                case UPDATE_CACHE_APP:
                    ...
                    View view = View.inflate(getApplicationContext(), R
                            .layout.linearlayout_cache_item, null);
                    ImageView iv_icon = (ImageView) view.findViewById(R
                            .id.iv_icon);
                    TextView tv_item_name = (TextView) view.findViewById(R
                            .id.tv_name);
                    TextView tv_memory_info = (TextView)view.findViewById(R
                            .id.tv_memory_info);
                    ImageView iv_delete = (ImageView) view.findViewById(R
                            .id.iv_delete);
                    ...
                    iv_delete.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            intent.setData(Uri.parse("package:" + cacheInfo
                                    .packagename));
                ...
                case CHECK_CACHE_APP:
                    mTv_claer.setText((String) msg.obj);
                    break;
                case CHECK_FINISH:
                    mTv_claer.setText("扫描完成");
                    break;
                case CLEAR_CACHE:
                    //从线性布局中移除所有的条目
    ...
    private void getPackageCache(String packageName) {
        IPackageStatsObserver.Stub mStatsObserver = new IPackageStatsObserver
                .Stub() {
            public void onGetStatsCompleted(PackageStats stats,
                ...
                if (cacheSize > 0) {
                    ...
                    try {
                        cacheInfo.name = mPm.getApplicationInfo(stats
                                .packageName, 0).loadLabel(mPm).toString();
                        cacheInfo.icon = mPm.getApplicationInfo(stats
                                .packageName, 0).loadIcon(mPm);
                    } catch (PackageManager.NameNotFoundException e) {
                        e.printStackTrace();
                    }
                    msg.obj = cacheInfo;
    ...
    public void clear(CacheInfo cacheInfo){
         //以下代码如果要执行成功则需要系统应用才可以去使用的权限
        //android.permission.DELETE_CACHE_FILES
       // CacheInfo cacheInfo = new CacheInfo();
        try {
            Class<?> clazz = Class.forName("android.content" +
                    ".pm.PackageManager");
            //2.获取调用方法对象
            Method method = clazz.getMethod("deleteApplicationCacheFiles",
                    String.class,IPackageDataObserver.class);
            //3.获取对象调用方法
            method.invoke(mPm, cacheInfo.packagename,new IPackageDataObserver
                    .Stub() {
            @Override
    ...
    private void initUI() {
        mPb = (ProgressBar) findViewById(R.id.pb_clear);
        mTv_claer = (TextView) findViewById(R.id.tv_text);
        ...
        btn_clear.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    //3.获取对象调用方法
                    method.invoke(mPm, Long.MAX_VALUE,new IPackageDataObserver
                            .Stub() {
                        @Override
