...
public class ProcessManagerActivity extends Activity implements View
        .OnClickListener {
    ...
    private void initUI() {
        ...
        lv_process_list.setOnItemClickListener(new AdapterView.
                OnItemClickListener() {
            //view选中条目指向的view对象
            @Override
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                if(position == 0 || position == mCustomerList.size()+1){
                    ...
                    if(mProcessInfo!=null){
                        if(!mProcessInfo.getPackageName()
                            ...
                            //checkbox显示状态切换
                            //通过选中条目的view对象,findViewById找到此条目指向的
                            // cb_box,然后切换其状态
                            /**
                             * 	//4.更新界面
                             ...
                             viewHolder.cb_box.setChecked(mProcessInfo.isCheck());
                             */
                            CheckBox cb_box = (CheckBox) view.findViewById(R
                                    .id.cb_box);
    ...
    private void clearAll() {
        ...
        //11,根据进程总数和剩余空间大小
        tv_process_count.setText("进程总数:"+mProcessCount);
        tv_memory_info.setText("剩余/总共"+Formatter.formatFileSize(this,
                mAvailSpace)+"/"+mStrTotalSpace);
        //12,通过吐司告知用户,释放了多少空间,杀死了几个进程,
        String totalRelease = Formatter.formatFileSize(this, totalReleaseSpace);
        //ToastUtil.show(getApplicationContext(), "杀死了"+killProcessList.size()
        // +"个进程,释放了"+totalRelease+"空间");
    ...
    class MyAdapter extends BaseAdapter {
        ...
        @Override
        public int getCount() {
            if (SpUtil.getBoolean(getApplicationContext(), ConstantValue
                    .SHOW_SYSTEM,false)){
            ...
            if(type == 0){
                ...
                if(convertView == null){
                    convertView = View.inflate(getApplicationContext(), R.layout
                            .listview_process_item, null);
                    holder = new ViewHolder();
                    holder.iv_icon = (ImageView)convertView.findViewById(R.id
                            .iv_icon);
                    holder.tv_name = (TextView)convertView.findViewById(R.id
                            .tv_name);
                    holder.tv_memory_info = (TextView) convertView.findViewById(R
                            .id.tv_memory_info);
                    holder.cb_box = (CheckBox) convertView.findViewById(R
                            .id.cb_box);
                    convertView.setTag(holder);
                }else{
